...
Формат числа определяет значение, которое в десятичной системе счисления будет равно +-1,мантисса * 2^(порядок-011111..11). Заметим, что перед запятой всегда 1 и она в явном виде не хранится.
